...
public class ChordsBassdemoHandle {
    ...
    [DllImport("ChordsBassdemo", CallingConvention = CallingConvention.StdCall)]
    ...
    internal ChordsBassdemoHandle(int key) {
        PluginKey = key;
    }
    
    ~ChordsBassdemoHandle() {
    ...
    private static ChordsBassdemoHandle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (ChordsBassdemoHandle)gch.Target;
    }
...
public class ChordsBassdemoHelper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static ChordsBassdemoHelper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("ChordsBassdemoHelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                ChordsBassdemoHelper helper = go.AddComponent<ChordsBassdemoHelper>();
                helper.Plugin = new ChordsBassdemoHandle(key);
                return helper;
            }
            return go.GetComponent<ChordsBassdemoHelper>();
        }
    }

    public ChordsBassdemoHandle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            ChordsBassdemoHandle.RegisterGlobalTransport(null);
        }
    }
}
